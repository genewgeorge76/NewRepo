"""
crm.py — Lead Pipeline CRM router.

Routes (under /api/v1/crm):
  GET   /leads                  list leads with stage/label filters
  PATCH /leads/{lead_id}/stage  update pipeline stage
  GET   /funnel                 stage funnel counts and win rate
"""
...
from ..core.cache import CRM_LEADS_TTL, KEY_CRM_FUNNEL, cache_get, cache_set, invalidate_crm_caches
        ...
        'leads': [
            {
                ...
                'email': l.email,
                'phone': l.phone,
                'service_type': l.service_type or l.service,
                'urgency': l.urgency,
                'score_label': l.score_label or l.tier,
...
async def update_stage(
    ...
    if body.pipeline_stage not in _VALID_STAGES:
        raise HTTPException(
            422,
            f'Invalid stage. Must be one of: {", ".join(sorted(_VALID_STAGES))}',
        )
